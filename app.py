...
def init():
    model_dir = snapshot_download('ljnyyds/zhangsan_say_law'
                                  , cache_dir='./')
...
def load_model():
    model = (
        AutoModelForCausalLM.from_pretrained("ljnyyds/zhangsan_say_law", trust_remote_code=True)
        .to(torch.bfloat16)
        .cuda()
    )
    tokenizer = AutoTokenizer.from_pretrained("ljnyyds/zhangsan_say_law", trust_remote_code=True)
